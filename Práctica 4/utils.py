...
def one_hot_encoding(Y):
    """
    Turns label vector Y (shape (m,)) into one-hot matrix (m, K).
    If labels are 1..10 with 10 representing digit 0, we map 10->0 index.
    """
    Y = np.asarray(Y).reshape(-1)
    # Determine number of classes (use max label)
    K = int(np.max(Y))
    # Map labels to indices 0..K-1 (special: 10 -> 0)
    idx = (Y % K).astype(int)
    onehot = np.zeros((Y.shape[0], K), dtype=float)
